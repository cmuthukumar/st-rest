		...
		stage('Setup TestProfiles')
		    ...
		    stage('Run Tests')
				...
				//st_ansibleImage.inside('-v /root/.ssh/:/root/.ssh/ -v /root/.aws/:/root/.aws/')
					...
					sh "cd ${workdir} && ansible-playbook -i inventories/${CloudProviders}/${params[0]}/ -i inventories/${CloudProviders}/${params[1]}/ -e as2_totalcreatehosts=600 -e as2_filespermin=250 -e as2_totalmins=120 -e as2_totalhosts=600 -e ftp_totalcreatehosts=300 -e ftp_filespermin=250 -e ftp_totalmins=120 -e ftp_totalhosts=300 -e sshftp_totalcreatehosts=300 -e sshftp_filespermin=250 -e sshftp_totalmins=120 -e sshftp_totalhosts=300 run_tests.yml "
		    ...
		    stage('Monitor Tests')
		        ...
		        //st_ansibleImage.inside('-v /root/.ssh/:/root/.ssh/ -v /root/.aws/:/root/.aws/')
			       //	{       							
					sh "cd ${workdir} && ansible-playbook -i inventories/${CloudProviders}/${params[0]}/ -i inventories/${CloudProviders}/${params[1]}/ -e as2_filespermin=250 -e as2_totalmins=120 -e ftp_filespermin=250 -e ftp_totalmins=120  -e sshftp_filespermin=250 -e sshftp_totalmins=120  monitor_tests.yml "
