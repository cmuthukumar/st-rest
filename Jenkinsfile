 ...
 withDockerRegistry([credentialsId: 'DockerCleoSysTest', url: 'https://hub.docker.com/r/cleo/ansible/']) {
          try{				
		    ...
		    stage('Run Tests')
				...
				st_ansibleImage.inside('-v /root/.ssh/:/root/.ssh/')
					...
					sh "cd ${workdir} && ansible-playbook -i inventories/${params[0]}/ -i inventories/${params[1]}/ -e as2_totalcreatehosts=1000 -e as2_filespermin=600 -e as2_totalmins=30 -e as2_totalhosts=1000 -e ftp_totalcreatehosts=500 -e ftp_filespermin=600 -e ftp_totalmins=30 -e ftp_totalhosts=500 -e sshftp_totalcreatehosts=500 -e sshftp_filespermin=600 -e sshftp_totalmins=30 -e sshftp_totalhosts=500 run_tests.yml "
		    ...
		    stage('Monitor Tests')
		        ...
		        st_ansibleImage.inside('-v /root/.ssh/:/root/.ssh/')
			       	{       							
					sh "cd ${workdir} && ansible-playbook -i inventories/${params[0]}/ -i inventories/${params[1]}/ -e as2_filespermin=600 -e as2_totalmins=30 -e ftp_filespermin=600 -e ftp_totalmins=30  -e sshftp_filespermin=600 -e sshftp_totalmins=30  monitor_tests.yml "
    ...
    withCredentials([[$class: 'StringBinding', credentialsId: 'doCredentials', variable: 'do_ap_token']]) {
        ...
        println "Creating Nodes for ${param}"
           sh "cd ${workdir} && ansible-playbook setup_topology.yml -c local -e machine_type=${param} -e do_api_token=${env.do_ap_token} -e username='jenkins' -e sshkey_name='st-versalex' "
