            ...
            for (String hostelem : vexHostProps.keySet()) {
                if ((vexHostProps.get(hostelem) != null) && (vexHostProps.get(hostelem).length() != 0) && (hostelem.toLowerCase().startsWith("host_")) && (!hostelem.toLowerCase().equalsIgnoreCase("host_defaultalias"))) {
                    if ((hostelem.toLowerCase().startsWith("host_address"))) {
                        System.out.println("Setting Up Host Addrs"+hostelem+"value is"+vexHostProps.get(hostelem.toString()));
    ...
    public void setupEachHost() throws Exception {
        ...
        try {
          ...
          //  setHostPath(vexHostProps.get("host_alias"));
            ...
            vexHostPath[0]=vexHostProps.get("host_alias");
            vexHostPath[1]=vexHostProps.get("mailbox_alias");
            vexHostPath[2]=vexHostProps.get("action_alias");
            System.out.println("Path value is "+Arrays.toString(vexHostPath));
        ...
        /*    if (vexHostProps.get("action_alias").length() > 0) {
          ...
          //  System.out.println("Host Addd:"+vexHostProps.get("host_address"));


         //   versalex.setProperty(ILexiCom.HOST, vexHostPath, "Address", props.getProperty(vexHostProps.get("host_address")));
            versalex.rename(ILexiCom.ACTION,vexHostPath,"send");
