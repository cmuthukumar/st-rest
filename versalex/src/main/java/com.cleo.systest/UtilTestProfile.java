...
public class UtilTestProfile {
        ...
        try {
            ...
            for (String hostslist : versalex.list(ILexiCom.HOST, vexHostPath)) {
                ...
                if (!hostslist.toLowerCase().contains("listener")) {
                    ...
                    vexHostPath[0] = versalex.remove(ILexiCom.HOST, vexHostPath);
    ...
    public void setProduct(String productName) {
        ...
        if (productName.toLowerCase().startsWith("harmony")) {
            vexproduct = 3;

        }

        if (productName.toLowerCase().startsWith("vltrader")) {
            vexproduct = 2;
        }

        if (productName.toLowerCase().startsWith("lexicom")) {
            vexproduct = 1;
        }
    ...
    public ILexiCom getVersalex() throws Exception {

        try {
            System.out.println("Creating Versalex vexproduct:" + this.vexproduct + "vexpath:" + this.vexHomePath);

            versalex = LexiComFactory.getVersaLex(this.vexproduct, this.vexHomePath, LexiComFactory.CLIENT_OR_SERVER);
            System.out.println("Product Version:" + versalex.getVersion());

            if (versalex != null) {
                System.out.println("Creating Versalex Done");
                return versalex;
    ...
    public int getTotalHosts() throws Exception {
        int total_hosts = 0;
        try {
            for (String hostslist : versalex.list(ILexiCom.HOST, this.vexHostPath)) {
                total_hosts = total_hosts + 1;
            }
            System.out.println("Total hosts available After" + total_hosts);


        } catch (Exception e) {

            System.out.println("Total Hosts available exception" + e.getMessage());
...

    {
        ...
        try {
            ...
            if (checkHostExists(hostname)) {
                ...
                if ((vexHostProps.get("service_alias") != "") && hostname.toLowerCase().startsWith("local listener")) {
                    vexHostPath[0] = hostname;
    ...
    public void setCustomProps(String type, String customProps, String[] vexpath) throws Exception {
        try {

            JSONObject vexCustomProps = new JSONObject(customProps);
            Iterator<?> keys = vexCustomProps.keys();
            while (keys.hasNext()) {
                ...
                if (type.toLowerCase().startsWith("mailbox"))
                    versalex.setProperty(ILexiCom.MAILBOX, vexpath, key, value);
                if (type.toLowerCase().startsWith("action"))
                    versalex.setProperty(ILexiCom.ACTION, vexpath, key, value);
            }

        } catch (Exception e) {

            System.out.println("Exception in setCustomMbxProps" + e.getMessage());
    ...
    public void setupSchedule() throws Exception {
        ...
        try {
            ...
            for (String hostelem : this.versalex.list(ILexiCom.HOST, vexHostPath)) {
                ...
                vexHostPath[0] = hostelem;

                for (String mbxelem : this.versalex.list(ILexiCom.MAILBOX, vexHostPath)) {
                    vexHostPath[1] = mbxelem;
                    System.out.println("Mailbox Elem is" + mbxelem);
                    for (String actionelem : this.versalex.list(ILexiCom.ACTION, vexHostPath)) {
                        vexHostPath[2] = actionelem;
