	...
	try:
		...
		for tp_ip in tphosts:
			...
			for i in range(start,end+1):
				print "ASYNC HOST",(host_name+str(i))
				async_result=pool.apply_async(as2_setup_proxy, ((host_name+str(i)),proxy_hosts,tphosts,tp_ip,master_ip,)).get()
				print "ASYNC RES",async_result
		...
		print "Exception on setup_as2_with_proxy",e
		return False,e

def as2_setup_proxy(hostname,proxy_hosts,tphosts,tp_ip,master_ip):
	try:
		setup_sender_cert(hostname,proxy_hosts[tphosts.index(tp_ip)],tp_ip,master_ip)
		setup_receiver_cert(hostname,tp_ip,proxy_hosts[tphosts.index(tp_ip)],master_ip)
		setup_connection(hostname,master_ip,sender_json_req)
		setup_connection(hostname,tp_ip,receiver_json_req)
	except Exception,e:
		print "System as2_setup_proxy Thread Error",e		
		
def as2_setup(hostname,server_hosts,tphosts,tp_ip,master_ip):
	try:
		setup_sender_cert(hostname,server_hosts[tphosts.index(tp_ip)],tp_ip,master_ip)
		setup_receiver_cert(hostname,tp_ip,server_hosts[tphosts.index(tp_ip)],master_ip)
		setup_connection(hostname,master_ip,sender_json_req)
		setup_connection(hostname,tp_ip,receiver_json_req)	
	except Exception,e:
		print "System as2_setup Thread Error",e
...
def setup_as2_without_proxy(host_name,server_hosts,tphosts,dataset):
	try:
		...
		for tp_ip in tphosts:
			...
			print "TP Hosts",tphosts[tphosts.index(tp_ip)]		
			for i in range(start,end+1):
			# as2_setup((host_name+str(i)),server_hosts,tphosts,tp_ip,master_ip)
				async_result=pool.apply_async(as2_setup, ((host_name+str(i)),server_hosts,tphosts,tp_ip,master_ip,)).get()
				start=start+int(host_pernode)
				end=end+int(host_pernode)
		pool.close()
		pool.join()
		...
		print "Exception on setup_as2_without_proxy",e
		return False,e
